...
function assertEqual(input, expRes) {
    console.log(input, expRes)
    if (input != expRes) {
        throw new Error(`Error: correct output: ${expRes}; current output: ${input}`);
    }
};
...
var Tests = [
    ...
    function test1() {
        it('Base Test 1', function() {
            var input = '4';
            var res = Calculate(input).value;
            assertEqual(Math.pow(res, 2), input);
          });
    },
    function test2() {
    ...
    function test7() {
        it('Random Number Test 3', function() {
            var input = ((Math.random() + 1) * 100000000).toString();
            var res = Calculate(input).value;
            assertEqual(Math.pow(res, 2), input);
          });
    },
